...
def fetch_sheet_rows(sheet_id, access_token=None):
    """
    Tries public API key first. Falls back to OAuth access_token if provided.
    Returns list of dicts with name/email/phone keys.
    """
    headers = {}
    if access_token:
...
@leads_bp.route("/connect_gsheet", methods=["POST"])
def connect_gsheet():
    """
    Save the Google Sheet URL against a company and do a first sync.
    Body: { company_id, sheet_url, access_token (optional, for private sheets) }
    """
    try:
        data = request.json or {}
        company_id = data.get("company_id")
        sheet_url = data.get("sheet_url", "").strip()
        access_token = data.get("access_token")  # from OAuth if private
        ...
        # Save sheet config to company doc
        compCollection.update_one(
            ...
            {"$set": {
                ...
                "gsheet_access_token": access_token,  # None for public
            }}
        )

        # Immediately sync
        rows = fetch_sheet_rows(sheet_id, access_token)
        inserted, skipped, duplicates = _import_rows(rows, company_id, user_id=None)
...
@leads_bp.route("/sync_gsheet/<company_id>", methods=["POST"])
def sync_gsheet(company_id):
    """Manual or auto sync. Pulls latest rows, inserts new leads only."""
    try:
        company = compCollection.find_one({"_id": ObjectId(company_id)})
        if not company:
            return jsonify({"error": "Company not found"}), 404
...
def _import_rows(rows, company_id, user_id):
    """Shared logic for CSV upload and sheet sync."""
    inserted = 0
    skipped = 0
    duplicates = []
        ...
        if existing:
            ...
            if uploader_id:
                try:
                    ...
                    pass
            duplicates.append({"name": name, "email": email, "phone": phone, "already_uploaded_by": uploader_name})
            continue
